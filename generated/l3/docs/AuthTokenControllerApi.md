# ods_sdk_L3.AuthTokenControllerApi

All URIs are relative to *http://localhost:8090*

Method | HTTP request | Description
------------- | ------------- | -------------
[**access_token**](AuthTokenControllerApi.md#access_token) | **POST** /auth/token | 


# **access_token**
> APIResponseAuthTokenResponse access_token(auth_token_request)

### Example


```python
import ods_sdk_L3
from ods_sdk_L3.models.api_response_auth_token_response import APIResponseAuthTokenResponse
from ods_sdk_L3.models.auth_token_request import AuthTokenRequest
from ods_sdk_L3.rest import ApiException
from pprint import pprint

# Defining the host is optional and defaults to http://localhost:8090
# See configuration.py for a list of all supported configuration parameters.
configuration = ods_sdk_L3.Configuration(
    host = "http://localhost:8090"
)


# Enter a context with an instance of the API client
with ods_sdk_L3.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = ods_sdk_L3.AuthTokenControllerApi(api_client)
    auth_token_request = ods_sdk_L3.AuthTokenRequest() # AuthTokenRequest | 

    try:
        api_response = api_instance.access_token(auth_token_request)
        print("The response of AuthTokenControllerApi->access_token:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AuthTokenControllerApi->access_token: %s\n" % e)
```



### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **auth_token_request** | [**AuthTokenRequest**](AuthTokenRequest.md)|  | 

### Return type

[**APIResponseAuthTokenResponse**](APIResponseAuthTokenResponse.md)

### Authorization

No authorization required

### HTTP request headers

 - **Content-Type**: application/json
 - **Accept**: */*

### HTTP response details

| Status code | Description | Response headers |
|-------------|-------------|------------------|
**200** | OK |  -  |

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to Model list]](../README.md#documentation-for-models) [[Back to README]](../README.md)

